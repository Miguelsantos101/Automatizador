...
class Changesets(AutomatedWebNavigation):
    ...
    def navigate(self):
        try:
            ...
            task = self.find_elements(
                By.XPATH,
                "//div[@aria-roledescription='Card' and descendant::div[contains(text(), 'Development')]]//span[@class='clickable-title']",
            )
            lista_de_changesets = []
            ...
            for i in range(len(task)):
                task_atual = task[i]
                task_atual.click()
                changesets = task[i].find_elements(
                    By.XPATH,
                    "//*[starts-with(@class, 'links-control-container')]//*[text() [contains(., 'Changeset')]]",
                )
                ...
                if len(changesets) == 0:
                    ...
                    while True:
                        procuraShowmore = task[i].find_elements(
                            By.XPATH,
                            "/html/body/div[6]/div[2]/div/div[2]/div/div/div/div[3]/div[1]/div/div[2]/div[2]/div[1]/div/div/div[2]/div/div/div[2]/div[2]/div/div/div",
                        )
                        if len(procuraShowmore) > 1:
                            botaoShowmore = procuraShowmore[1].find_element(
                                By.XPATH,
                                "/html/body/div[6]/div[2]/div/div[2]/div/div/div/div[3]/div[1]/div/div[2]/div[2]/div[1]/div/div/div[2]/div/div/div[2]/div[2]/div/div/div[2]/div[1]/div/div[1]",
                            )
                            botaoShowmore.click()
                            changesets = task[i].find_elements(
                                By.XPATH,
            ...
            lista_de_changesets = self.remover_parte_especifica(
                lista_de_changesets, "Changeset "
            )
